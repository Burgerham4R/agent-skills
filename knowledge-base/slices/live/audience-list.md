...
audienceStore.fetchAudienceList(completion:)
...
| 概念 | 说明 |
|------|------|
| **LiveAudienceStore** | 观众信息管理模块，每个直播间通过 `create(liveID:)` 创建独立实例 |
...
4. **直播结束或退出时销毁 audienceStore** — 调用 `audienceStore = nil`（或等效的销毁操作）释放订阅和内部资源，防止内存泄漏。
...
| 错误码 | 描述 | 处理建议 |
|--------|------|----------|
| `-1002` | 未登录 | 确保 `LoginStore.shared.login` 成功后再操作 |
| 列表返回空 | 正常情况：当前直播间无观众 / 未进房就调用 | 确认已进房；检查是否在 `joinLive.success` 后调用 |
| 人数显示不准确 | 频控丢弃消息（正常现象） | 仅展示，不依赖精确值；可提示"约 X 人在看" |
| 观众列表不更新 | 未订阅 `liveAudienceEventPublisher` | 检查事件订阅是否在 `AnyCancellable` 中持有 |
...
观众进离场后列表不刷新
├── 是否订阅了 liveAudienceEventPublisher？
│       → 否 → 添加订阅
│       → 是 → 检查 AnyCancellable 是否被存储（未存储则立即释放）
└── 事件回调是否在主线程刷新 UI？ → 否 → 添加 .receive(on: DispatchQueue.main)
